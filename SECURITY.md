...
| LLM03 | Supply Chain | **Addressed.** `pip-audit` scans `requirements.txt` in CI (clean as of this writing), and the file pins every dependency with `==` at the exact version this project's own CI has actually run and passed against (`openai==2.30.0`, `anthropic==0.97.0`, `pypdf==6.16.1`, `jsonschema==4.26.0`) — a fresh install six months from now is auditing the same target it was audited against today, not silently drifting to whatever's newest that day. `DECISION-D66.md` records the fix; this row is updated to match it (an earlier version of this row said the file still used `>=` — stale the moment D66 shipped, corrected here). |
