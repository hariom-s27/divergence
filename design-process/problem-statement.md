...
### Step 3 deliverable · **v2** · 4 August 2026

> **What changed in v2.** The Step 4 academic research produced three things stronger than v1 had:
> 1. **The correct legal term.** Ours is a **lacuna** — a gap where no rule exists — not "open texture," which means a rule exists but is vague. Rule 11UA doesn't say something imprecise about digital assets; it says nothing.
> 2. **A third uncertainty category.** The literature has the model's ignorance and the world's randomness. Ours is neither: the world has not *decided*. This is what "nobody knows" means technically, and it makes the one-liner load-bearing rather than rhetorical.
> 3. **Auditing already legitimises the output.** A *disclaimer of opinion* is a formal professional product meaning "we cannot form a view." That is the complete answer to "does saying it's unclear actually help?"
>
> **Design rule for v2:** the one-liner and 30-second version stay free of jargon. The depth goes into the 3-minute version and the Q&A reserve. Never make a generalist judge learn a word to follow the pitch.
...
There is no prescribed method in Indian tax law for valuing a digital asset in rupees at a point in time. The rule that covers property, jewellery and shares was never extended to cover this.

And this is not a rule that's vague — it's a rule that was never written. Lawyers have a word for that: a **lacuna**. A gap. It matters, because you cannot resolve a gap by reading the law more carefully.

Meanwhile the one official exchange rate is published once a day, on weekdays only, from a random five-minute window late in the morning. Her payment settled at three in the morning on a Sunday, when no official rate exists at all.
...
And it isn't the model's fault in the ordinary sense. Machine learning knows two kinds of uncertainty: the model doesn't know something, or the world is random. A coin flip is random — but it still *has* a fifty-per-cent answer. Here there is no answer of any kind, because nobody has decided yet. The model has no way to represent that, so it reaches for the nearest plausible number and delivers it in the voice of settled fact.
...
And "we cannot form a view" is not a failure state in her world. Auditors have a formal opinion type for exactly this — a disclaimer of opinion. An entire regulated profession is paid to say it. We are giving that vocabulary to an AI.
...
## "Isn't this just LLM abstention? That's a known research area."
> Abstention research asks whether a model will decline to answer. That's a yes/no decision about the model's own confidence. We're doing something different: the question genuinely has no answer, so declining isn't enough — she still has to file a number. We resolve it every defensible way and price the disagreement, which abstention research doesn't attempt.

## "Isn't this Hart's open texture? That's been discussed since 1961."
> Open texture is when a rule exists but is vague — "drive safely." Ours is a lacuna: no rule exists at all. Rule 11UA doesn't say something imprecise about digital assets, it says nothing. Different problem, different response. You can resolve vagueness by interpretation; you cannot interpret your way out of a gap.

## "Hasn't someone already used LLMs on legal ambiguity?"
> Yes, and we cite it. There's 2025 work using LLMs to annotate open-textured terms in regulatory documents. That labels which words in a statute are vague. We take a real transaction, resolve it, and report what the gap costs in rupees. They annotate the text; we resolve the case.

## "How is this different from just showing an error bar?"
> An error bar implies a true value we're uncertain about. Here there is no true value. The two numbers aren't estimates of one figure — they're two different legitimate answers, and which one applies is a decision nobody has made yet.

---

# PART 3B — Q&A DEPTH RESERVE
*Do not put these in the pitch. Hold them for when a judge digs.*

| If a judge asks about… | Reach for |
|---|---|
| The uncertainty taxonomy | Aleatoric (world randomness) / epistemic (model ignorance) / **normative indeterminacy** (no authority has decided) — the third isn't in the standard dichotomy |
| Whether the field knows this is a gap | A 2026 formal-reasoning legal AI paper lists as its own limitation that it *"cannot capture legal provisions that deliberately incorporate normative ambiguity"* |
| Software engineering parallels | Hoare's billion-dollar mistake — implicit null lets absence masquerade as a value. Our gap detector is an `Option` type for legal reasoning: `insufficient_evidence` cannot be silently coerced into a default |
| How you report a value with no observable price | IFRS 13 / ASC 820 fair value hierarchy — Level 1/2/3 by input observability, with mandatory sensitivity disclosure. We tag conclusions the same way |
| Whether "we don't know" is a real product | Auditing's disclaimer of opinion. Metrology's uncertainty budget. Weather's calibrated probabilities. Three professions that made it work |
| How you know you're not just abstaining on everything | We measure and publish our abstention rate. A system that says "unclear" about everything is noise |
| Scope beyond crypto | Twelve domains carry the same lacuna problem — medical dosing outside trial populations, "reasonable and customary," transfer pricing ranges, fair use, legitimate interest, EU AI Act risk classification |
