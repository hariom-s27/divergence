...
**⚠️ CORRECTED 6 Aug (C46) — we were leaning on the wrong limb.** Limb (b) is about the *Assessing Officer* estimating because the taxpayer's accounting method does not allow income to be deduced. That is not our situation.

**Our limb is s.439(8)(a) — a bona fide explanation with all material facts disclosed.** And it is the better one, because it is where the case law actually sits: the ITAT has held that penalty under this provision is *"discretionary, not automatic, and cannot be levied where the addition is based on a difference in legal view or computational methodology rather than suppression of income,"* and that *"a bona fide, fully disclosed, debatable tax position cannot be penalised merely because the Revenue disagrees."* A valuation-method disagreement is a computational-methodology difference, described in a tribunal's own words.

Supporting authority: *Sangrur Vanaspati Mills* (SC — no penalty on estimate-based additions); *T. Ashok Pai v. CIT* (SC — bona fide explanation discharges the burden; penalty is quasi-criminal, mens rea essential); *Samson Perinchery*, 392 ITR 4 (Bom HC — penalty cannot be levied on a different limb than the one initiated).
