...
- **Next, once verifiable**: paste as a few-shot block into
  `03-income-tax.md`'s own prompt, run the pipeline for real, and confirm
  — via `eval/score.py` and a fresh `node5_adversarial.py` pass — that
  adding them doesn't change M1–M4 on the six cases and does reduce (not
  just relocate) the rate at which node 5 has to catch this same shape a
  fourth time. Not attempted here without a way to verify the result.
