...
run the pipeline end to end, and all 30 dated design decisions merged into
...
| `DECISION-D41.md` … `DECISION-D70.md` | Thirty dated documents, each recording one real design decision or bug — merged into `START-HERE.md`, kept individually for full detail |
...
| `injection_scanner.py`, `cases/ADV1-injection/` | Prompt-injection defence for the one node that reads untrusted text — a CI-gated pattern scanner (11 phrase families plus hidden/non-printing character detection, D70) plus nonce spotlighting in `node1_extract.py`, verified against a real adversarial case (D62); findings now stored at `_meta.input_integrity` and rendered as their own visible section on the disclosure page, not just folded into prose |
